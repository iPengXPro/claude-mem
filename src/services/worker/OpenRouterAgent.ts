...
// OpenRouter API endpoint (or DashScope compatible endpoint)
// Default is set to DashScope OpenAI-compatible endpoint for Chinese users
const DEFAULT_OPENROUTER_BASE_URL = 'https://coding.dashscope.aliyuncs.com/v1/chat/completions';
...
export class OpenRouterAgent {
  ...
  private async queryOpenRouterMultiTurn(
    ...
    const apiUrl = baseUrl || DEFAULT_OPENROUTER_BASE_URL;
  ...
  private getOpenRouterConfig(): { apiKey: string; model: string; siteUrl?: string; appName?: string; baseUrl?: string } {
    ...
    // API Base URL: from settings or default (DashScope compatible)
